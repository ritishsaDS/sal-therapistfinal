...
final String cname;
  ...
  CallPage({Key key,this.cname,this.dif,this.session,this.type, this.channelName,this.id, this.name, this.role}) : super(key: key);
...
class _CallPageState extends State<CallPage> {
  ...
  void startTimer() {
    ...
    _timer = new Timer.periodic(
      oneSec,
        ...
        if (_start == 0) {
          setState(() {
            d = Duration(minutes: 0, seconds: _start);
            d.toString();
            _start++;
        ...
        child: Row(
          ...
          children: <Widget>[
            RawMaterialButton(
              ...
              child: Icon(
                callshow? Icons.missed_video_call_outlined : Icons.video_call_outlined,
            ...
            _start>=600? RawMaterialButton(
              ...
              onPressed: (){
                Fluttertoast.showToast(msg: "Wait for few minutes please!");
              },
              child: Icon(
                Icons.call_end,
                color: Colors.white,
                  ...
                  child: Container(
                      ...
                      child: Text(
                        isParent == false
                            ?  username.toString()
                            :
                        (_getRenderViews().length==2)?widget.cname.toString():"",
                        style: TextStyle(
  ...
  _getArogaToken() async {
    ...
    await AgoraRepo.getAgoraToken(widget.channelName, widget.session, widget.type);
   ...
   startacall() async {
    ...
    if(prefs.getString("type")=="Therapist"){
      uri =
      'https://yvsdncrpod.execute-api.ap-south-1.amazonaws.com/prod/therapist/appointment/start?appointment_id=${widget.channelName}';

    }
    else if(prefs.getString("type")=="Listener"){
      uri =
      'https://yvsdncrpod.execute-api.ap-south-1.amazonaws.com/prod/listener/appointment/start?appointment_id=${widget.channelName}';

    }
    else if(prefs.getString("type")=="Counsellor"){
      uri =
      'https://yvsdncrpod.execute-api.ap-south-1.amazonaws.com/prod/counsellor/appointment/start?appointment_id=${widget.channelName}';
    ...
    try {
      print("hweinnowjhbhhen");
      print(response.data);
      if (response.data != null) {
        setState(() {
          username=prefs.getString("firstname");
print("JKHNFJDLSD"+response.data.toString());
        });
      } else {
  ...
  endacall() async {
    ...
    if(prefs.getString("type")=="Therapist"){
      uri =
      'https://yvsdncrpod.execute-api.ap-south-1.amazonaws.com/prod/therapist/appointment/end?appointment_id=${widget.channelName}';

    }
    else if(prefs.getString("type")=="Listener"){
      uri =
      'https://yvsdncrpod.execute-api.ap-south-1.amazonaws.com/prod/listener/appointment/end?appointment_id=${widget.channelName}';

    }
    else if(prefs.getString("type")=="Counsellor"){
      uri =
      'https://yvsdncrpod.execute-api.ap-south-1.amazonaws.com/prod/counsellor/appointment/end?appointment_id=${widget.channelName}';
